...
const ChooseCircle = ({ CircleType, className, clickEvent }: { CircleType: any, className?: string, clickEvent?:any }) => {
  const parentClass = 'md:w-[11rem] md:h-[11rem] w-[8rem] h-[8rem] rounded-full flex items-center justify-center border-b-8 border-t-8 border-t-transparent border-opacity-50 border-dark';
  ...
  const imgClass = 'group-hover:rotate-45 absolute transition-transform duration-300 ease-in-out';
        ...
        CircleType === Type.Rock ? (
          <div className={`${parentClass} bg-red-500`} onClick={clickEvent}>
            <div className={defaultClass}>
              <img src="../../public/assets/icon-rock.svg" width={70} height={70} alt={Type[0]} className={imgClass} />
            </div>
          </div>
        ) : null
      }
      {
        CircleType === Type.Paper ? (
          <div className={`${parentClass} bg-blue-500 `} onClick={clickEvent}>
            <div className={defaultClass}>
              <img src="../../public/assets/icon-paper.svg" width={70} height={70} alt={Type[1]} className={imgClass} />
            </div>
          </div>
        ) : null
      }
      {
        CircleType === Type.Scissors ? (
          <div className={`${parentClass} bg-orange-400`} onClick={clickEvent}>
            <div className={defaultClass}>
              <img src="../../public/assets/icon-scissors.svg" width={70} height={70} alt={Type[2]} className={imgClass} />
            </div>
          </div>
        ) : null
      }
